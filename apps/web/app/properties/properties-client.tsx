...
export function PropertiesClient({ initialListings }: { initialListings: PublicListing[] }) {
  const [listings, setListings] = useState<PublicListing[]>(initialListings);
  const [search, setSearch] = useState('');

  useEffect(() => {
    const q = search ? `?search=${encodeURIComponent(search)}` : '';
    publicApi<PublicListing[]>(`/public/listings${q}`).then(setListings).catch(console.error);
  }, [search]);

  return (
    <PublicShell>
      <div className="mx-auto max-w-6xl px-4 py-8">
        <h1 className="text-3xl font-semibold text-[#1a2744]">Properties for sale in Abuja</h1>
        <p className="mt-1 text-slate-600">
          From Triple A&apos;s FOR SALE catalog — Abuja &amp; environs. Browse duplexes, terraces, and
          more.
        </p>
