...
export default function ArtisanApplyPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await publicApi<{ message: string }>('/artisans/apply', {
        method: 'POST',
        body: JSON.stringify({
          ...form,
          trades: form.trades
      ...
      setOk(res.message);
      setTimeout(() => router.push('/login?next=/artisan'), 2500);
    } catch (err) {
      ...
      <div className="mx-auto max-w-lg px-4 py-10">
        <div className={`${CARD} p-6`}>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Trade accounts are separate from property-client / seeker logins. Admin approves before
            you can take assigned jobs. Phone numbers stay private — never shown in chat.
          </p>
